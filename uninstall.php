...
{
	if (!empty($modSettings['currentAttachmentUploadDir']))
		$path = $modSettings['attachmentUploadDir'][$file['id_folder']];
	else
		$path = $modSettings['attachmentUploadDir'];
	$path = $path . '/' . $file['id_attach'] . '_' . $file['file_hash'];
	if (file_exists($path . '.321') && is_writable($path . '.321'))
		rename($path . '.321', $path);
}
...
//==========================================================================
// Figure out where the attachment path(s) are:
$tmp = substr($db_prefix, strrpos($db_prefix, '.') + 1);
$tblchk = $smcFunc['db_query']('', 'show tables like "' . $tmp . 'pm_attachments"', array());
...
//==========================================================================
// Figure out where the attachment path(s) are:
$tblchk = $smcFunc['db_query']('', 'show tables like "' . $tmp . 'aeva_settings"', array());
while ($row = $smcFunc['db_fetch_assoc']($tblchk))
{
